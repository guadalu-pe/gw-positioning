...
const STEPS = ['Headline & Values', 'Content & Time', 'Contact & Publish'];

const sections = [
  { num: 1, title: 'Your Headline', tip: 'This is your headline. Give a snapshot of who you are so people can decide whether to read on. Show the core elements of your journey and what you want to be known for.', fields: [
  ...
  { num: 2, title: 'Your Professional Values', tip: 'Share 2–3 values that define how you approach your work. You can use a quote + brief explanation format.', fields: [
  ...
  { num: 3, title: 'Your Content or Work', tip: 'For writers: showcase your latest articles or projects. For visual thinkers: use this space for design or video work.', fields: [
  ...
  { num: 4, title: 'How You Spend Your Time', tip: 'Share how you spend your time professionally (and optionally personally) so people who want to work with you know what to expect.', fields: [
  ...
  { num: 5, title: 'How to Reach You', tip: 'Always have a section for people to get in touch. A Calendly link, email or contact form works great.', fields: [
  ...
  { num: 6, title: 'Your CTA', tip: 'What\'s the ONE place you want to direct people? Newsletter? Portfolio? Book a call?', fields: [
...
function SectionBlock({ section, fields, onUpdate }) {
  return (
    <div className="card" style={{ marginBottom: '20px' }}>
      ...
      <div className="callout tip" style={{ marginBottom: '16px' }}>
        ...
        <div className="callout-content"><p>{section.tip}</p></div>
      </div>
      {section.fields.map(f => (
        <div key={f.key} className="field-group">
          <label className="field-label">{f.label}</label>
          {f.type === 'text'
            ? <input type="text" placeholder={f.placeholder} value={fields[f.key] || ''} onChange={e => onUpdate(f.key, e.target.value)} />
            : <textarea rows={3} placeholder={f.placeholder} value={fields[f.key] || ''} onChange={e => onUpdate(f.key, e.target.value)} />}
        </div>
      ))}
    </div>
  );
...
export default function Stage4({ data, onUpdate, onComplete, isCompleted }) {
  ...
  useEffect(() => { onUpdate({ portfolio: fields }); }, [fields]);
  ...
  return (
    <div className="stage-container">
      <div className="stage-header">
        <div className="stage-badge"><Icon name="palette" size="13px" /> Stage 4</div>
        <h1 className="stage-title">Creating Your Generalist Portfolio</h1>
        <div className="stage-meta">
          <span className="stage-time"><Icon name="schedule" size="15px" /> Est. 1–2 hours</span>
        </div>
      </div>
      ...
      {step === 0 && (
        <>
          ...
          <div className="outcomes-box">
            ...
            <ul><li>Built your very own personal portfolio, ready to show the world</li></ul>
          </div>
          <div className="callout action" style={{ marginBottom: '24px' }}>
            <Icon name="auto_fix_high" className="callout-icon" />
            <div className="callout-content">
              <h4>Action</h4>
              <p>Draft your portfolio content below, then transfer it to your website tool of choice.</p>
            </div>
          </div>
          {stepSections[0].map(s => <SectionBlock key={s.num} section={s} fields={fields} onUpdate={update} />)}
        </>
      )}

      {step === 1 && (
        <>
          {stepSections[1].map(s => <SectionBlock key={s.num} section={s} fields={fields} onUpdate={update} />)}
        </>
      )}

      {step === 2 && (
        <>
          {stepSections[2].map(s => <SectionBlock key={s.num} section={s} fields={fields} onUpdate={update} />)}

          <h3 style={{ fontSize: '15px', fontWeight: '700', color: 'var(--gray-700)', margin: '28px 0 12px' }}>Recommended Portfolio Tools</h3>
          ...
          <div className="callout tip">
            ...
            <div className="callout-content">
              <h4>Next step</h4>
              <p>Once you're happy with your draft content above, transfer it into your chosen tool and publish. Then share it in the <strong>#heckyeah</strong> channel in the Generalist World community!</p>
            </div>
          </div>
        </>
      )}
