...
const STEPS = ['Your Framework', 'Your One-Liner', 'AI Tagline'];

const frameworkItems = [
  { letter: 'N', word: 'Name', action: 'Clearly introduce yourself and your professional identity.', purpose: 'Create a memorable first impression.', example: '"I\'m Alex, a multidisciplinary problem solver specializing in cross-sector innovation."', placeholder: 'I\'m [your name], a [how you describe yourself]...', key: 'name' },
  { letter: 'S', word: 'Same', action: 'Compare your approach or skillset to something familiar.', purpose: 'Quickly communicate your unique value proposition.', example: '"I\'m like a Swiss Army knife for organizations needing adaptive talent."', placeholder: 'I\'m like a [analogy] for [who you help]...', key: 'same' },
  { letter: 'F', word: 'Fame', action: 'Highlight your distinctive skills, achievements, or unique perspective.', purpose: 'Establish credibility and differentiation.', example: '"I\'ve successfully bridged challenges across tech, healthcare, and nonprofit sectors."', placeholder: 'I\'ve [your key achievements or experiences]...', key: 'fame' },
  { letter: 'A', word: 'Aim', action: 'Share your current professional focus or short-term objectives.', purpose: 'Demonstrate intentionality and forward momentum.', example: '"My aim is to develop innovative solutions connecting emerging tech with social impact."', placeholder: 'My current focus is [your aim]...', key: 'aim' },
  { letter: 'G', word: 'Game', action: 'Articulate your broader career vision or long-term aspiration.', purpose: 'Inspire interest and show strategic thinking.', example: '"My ultimate goal is to become a catalyst for interdisciplinary collaboration."', placeholder: 'My bigger vision is [your game]...', key: 'game' },
];
...
export default function Stage3({ data, onUpdate, onComplete, isCompleted }) {
  ...
  async function handleGenerate() {
    ...
    try { const result = await generateTaglineViaAPI(apiKey, prompt); setAiResult(result); }
  ...
  return (
    <div className="stage-container">
      <div className="stage-header">
        <div className="stage-badge"><Icon name="edit_note" size="13px" /> Stage 3</div>
          ...
          {frameworkItems.map(item => (
            <div key={item.key} className="framework-item">
              ...
              <div className="framework-item-body">
                ...
                <textarea rows={3} placeholder={item.placeholder} value={framework[item.key] || ''} onChange={e => updateFramework(item.key, e.target.value)} />
              </div>
            </div>
          ))}
        </>
        ...
        <div className="ai-box">
          ...
          {aiResult && (
            <div className="ai-result">
              ...
              <div style={{ whiteSpace: 'pre-wrap', fontSize: '14px', lineHeight: '1.7', color: 'var(--gray-800)' }}>{aiResult}</div>
              <div style={{ marginTop: '12px', display: 'flex', gap: '8px' }}>
                <button className="btn btn-secondary btn-sm" onClick={() => navigator.clipboard.writeText(aiResult)}>
                  <Icon name="content_copy" size="14px" /> Copy Result
                </button>
                <button className="btn btn-ghost btn-sm" onClick={() => { setAiResult(''); onUpdate({ ...data, aiResult: '' }); }}>Clear</button>
              </div>
            </div>
          )}
        </div>
